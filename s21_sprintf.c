...
#define IF_FROM_FIRST_ZERO  //используется для того, чтобы правильно
                            //отформатировать число, учитывая его начальные нули
                            //в дробной части.
...
#define ADD_SIGN  //   добавляет знак плюса (+) или пробел в начале строки, если
                  //   это необходимо согласно установленным флагам
                  //   форматирования
...
#define ADD_ACCURACY_TO_NUMBER  //  добавит дополнительные нули, чтобы достичь
                                //  требуемой точности.
if (accuracy == -1)
  accuracy = 1;
else
...
#define PARSE_FLAGS_ETC  // ПАРСИМ ФОРМАТ СТРОКИ. УСТАНОВКА ФЛАГОВ И ПАРАМЕТРОВ
int stopParse = 0;
while (format[i] != '\0' && stopParse == 0) {
  i++;
  int didSomething = 0;  //  выполнено какое-либо действие?
  ...
  if (parcerPhase == 1) {  // обрабатываются различные элементы формата
    if (format[i] == '*')
  ...
  if (parcerPhase == 2) {  // обработка точности формата.
    if (format[i] == '.' && !phaseLock)
      ...
      didSomething = 1, error = 0, accuracy = accuracy * 10 + format[i] - '0';
  }
  if (s21_strchr(ARG_LEN, format[i]) &&
      parcerPhase == 3)  // Проверяет, соответствует ли символ длине(L,h,l)
    arg_len = format[i], didSomething = 1;
  if (error == 2 || s21_strchr(SPECIFIERS, format[i]) || format[i] == '\0' ||
      parcerPhase == 4) {
...
#define G_TO_STR  // обрабатывает число с плавающей точкой
...
#define E_TO_STR  //   форматирования числа с плавающей точкой в
...
#define D_TO_STR  //   форматирование целых чисел
tempArray = calloc(100, sizeof(char));
if (tempArray != NULL) {
  long long num;
  if (arg_len == 'h')
...
#define O_TO_STR  //        форматирование в восьмеричной системе счисления
...
#define X_TO_STR  //      форматирование в шестнадцатеричной системе счисления
tempArray = calloc(150, sizeof(char));
if (tempArray != NULL) {
  if (accuracy == -1) accuracy = 1;
  for (int i1 = 0; i1 < 150; i1++) tempArray[i1] = '\0';
  ...
  if (modifiedOutput && (num != 0)) {
    ...
    put_char_in_front_of_str(tempArray, outputCh, s21_strlen(tempArray), index);
    put_char_in_front_of_str(tempArray, '0', s21_strlen(tempArray), index);
  }
}

#define P_TO_STR  //форматирование указателя - Преобразует значение указателя в
                  //шестнадцатеричную форму и добавляет 0x в начало
tempArray = calloc(150, sizeof(char));
if (tempArray != NULL) {
...
#define U_TO_STR  //форматирование беззнаковых целых чисел - Преобразует это
                  //число в строку и применяет настройки точности
tempArray = calloc(150, sizeof(char));
if (tempArray != NULL) {
  unsigned long long num;
  if (accuracy == -1) accuracy = 1;
...
#define F_TO_STR  //   Преобразует это число в строку и применяет настройки
                  //   точности
tempArray = calloc(100, sizeof(char));
if (tempArray != NULL) {
  long double num;
...
#define C_TO_STR  //форматирование символа - Сохраняет символ в строку.
  tempArray = calloc(20, sizeof(char));
  if (tempArray != NULL)
    tempArray[0] = (char)va_arg(list, int), tempArray[1] = '\0';

#define PERCENT_TO_STR  //   форматирование символа процента
  tempArray = calloc(20, sizeof(char));
  if (tempArray != NULL) tempArray[0] = '%', tempArray[1] = '\0';
  break;

#define STR_TO_STR  //   форматирование строк
...
#define N_TO_STR  //запись длины сформированной строки
  tempArray = calloc(150, sizeof(char));
  if (tempArray != NULL) {
    for (int i1 = 0; i1 < 150; i1++) tempArray[i1] = '\0';
...
#define SPECIFIERS_SWITCH
  ...
  static void add_to_str(char *tempArray, int leftFlag, const char *format,
                         int i, int *j, char *str, int width, int zeroFlag,
                         int old_len);
  ...
  int s21_sprintf(char *str, const char *format, ...) {
    ...
    int error = 0;
    for (; format[i] != 0 && error != 2;
         i++, j++) {  // Обработка строки формата
      str[j] = 0;
      if (format[i] != '%')
        str[j] = format[i];
      else {  //Обработка спецификаторов формата:
  ...
  static void int_to_string(
      long long num,
      char *result_str) {  //преобразует целое число типа long long в строку
    if (num == 0)
      result_str[0] = '0';
    else {
      int i = 0, index = 0;
  ...
  static void float_to_string(
      long double num, char *result_str, int from_first_not_zero, int accuracy,
      int keepZero) {  // преобразует число с плавающей точкой в строку
    ...
    for (int i = s21_strlen(secondArray); i < accuracy; i++)
      secondArray[i] = '0';
  ...
  static void float_to_e(
      long double num, char *result_str, int isSmol, int accuracy,
      int supressZeros) {  // берет число с плавающей точкой и преобразует его в
                           // удобочитаемую экспоненциальную форму
    ...
    if (vector) {
      ...
      int leftAlreadyAcc =
          (leftPartOfSecond > (long long)pow(10, accuracy) - 1);
    ...
    s21_sprintf(result_str, "%c%s%c%s%c%c%s", isNegative ? '-' : '\0',
                firstArray, dot, secondArray, isSmol ? 'e' : 'E',
                vector ? '+' : '-', counterStr);
  }

  static unsigned int convert_to_oct(
      unsigned int num) {  // Конвертирует целое число num в восьмеричную
                           // систему счисления
  ...
  static void convert_to_hex(
      unsigned long long num, char *result,
      int isSmol) {  //Преобразует целое число num в шестнадцатеричную форму
    int counter = 0;
    do {
  ...
  static void put_char_in_front_of_str(
      char *str, char ch, int len,
      int start) {  // Вставляет символ ch в начало строки str, сдвигая
                    // остальные символы вправо
    for (int j = len; j > start; j--) str[j] = str[j - 1];
    str[0] = ch;
  }

  void get_zeros(int accuracy, int *zeros,
                 long double *second) {  // Определяет количество нулей, которые
